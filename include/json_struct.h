...
template <typename T>
static inline void std_from(const T &d, Token &token, Serializer &serializer)
{
  std::string str_temp = std::to_string(d);
  int size = str_temp.length();
  char *buf = new char[size + 1];
  memset(buf, 0, size + 1);
  memcpy(buf, str_temp.c_str(), size);
...
struct TypeHandler<double>
{
  ...
  {
    std::string str_temp(context.token.value.data, context.token.value.size);
    try
    {
      to_type = std::stod(str_temp);
    }
    catch (...)
    {
...
struct TypeHandler<float>
{
  ...
  static inline void from(const float &d, Token &token, Serializer &serializer)
  {
    std_from(d, token, serializer);
  }
};
